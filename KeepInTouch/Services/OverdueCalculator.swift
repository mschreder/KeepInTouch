...
enum OverdueCalculator {
    static let dueSoonWindowDays = 3

    static func dueDate(for person: Person) -> Date {
    ...
    static func daysUntilDue(for person: Person, now: Date = Date()) -> Int {
    ...
    static func status(for person: Person, now: Date = Date()) -> ContactStatus {
    ...
    static func sorted(_ people: [Person], now: Date = Date()) -> [Person] {
